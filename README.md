...
棋盘休眠字显示 Zz；激活后隐藏内部边界，显示一个双格外框，保留左右两个字。待放置栏不显示休眠符号。拖动任一字立即解除关系，放下后重新判断；仍使用现有移动/交换规则，铲子限制不变。每个激活组合只有一个攻击冷却，采用双格中心索敌；共同测试属性在 src/config/heroes.ts：伤害35、射程230、间隔1200毫秒。未加入武将等级、EXP或技能。
