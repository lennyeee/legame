...
武将字始终为独立对象，不再合并成单格完整武将。src/systems/heroActivation.ts 从当前棋盘派生 HeroLink，记录姓名、左右格下标、两个字的对象引用和中心点。仅同一行、中心间距等于格子尺寸、从左到右符合配方的两个字激活：赵｜云、关｜羽、张｜飞。反序、纵向、对角线、隔格及待放置栏均不激活。

棋盘休眠字显示 Zz；激活后隐藏内部边界，显示一个双格外框，保留左右两个字。待放置栏不显示休眠符号。拖动任一字立即解除关系，放下后重新判断；仍使用现有移动/交换规则，铲子限制不变。每个激活组合只有一个攻击冷却，采用双格中心索敌；共同测试属性在 src/config/heroes.ts：伤害35、射程230、间隔1200毫秒。武将成长见下方 v0.47 规则；未加入技能。
